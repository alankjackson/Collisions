...
badspots <- 
  citypedslatlng %>%
  group_by(latlon, Rpt_Street_Name, Severity) %>%
  summarise(total=n())
...
CenterModtown <- c(-95.388, 29.744)
CenterSharptown <- c(-95.534, 29.703)
CenterNorthline <- c(-95.378, 29.849)
CenterAcres <- c(-95.471, 29.859)

gmap = get_map(location=CenterDowntown, source="google", zoom=15)
...
badspots_bg <-
  ...
  summarise(total=n()) %>%
  filter(total>2) %>%
  filter(between(Latitude,CenterDowntown[2]-0.01, CenterDowntown[2]+0.01) && between(Longitude,CenterDowntown[1]-0.01, CenterDowntown[1]+0.01))

badspots <- 
  citypedslatlng %>%
  mutate(Injury = case_when(
    Severity %in%  c("SUSPECTED SERIOUS INJURY", "FATAL") ~ "Serious",
    TRUE                      ~ "Non-Serious"
  )) %>%
...
badmap <- badspots %>% filter(between(Latitude,CenterDowntown[2]-0.01, CenterDowntown[2]+0.01) && between(Longitude,CenterDowntown[1]-0.01, CenterDowntown[1]+0.01)) 
ggmap(gmap, extent='normal', maprange=FALSE, show.legend=FALSE) %+% badmap + aes(x = Longitude, y = Latitude) +
  geom_point(data=badspots_bg, aes(size=total*2, x = Longitude, y = Latitude), color="yellow", alpha=.5) %+% badmap +
  geom_point(aes(size=total, x = Longitude, y = Latitude, color=total)) +
facet_wrap(TimeOfDay ~ Injury) +
  #guides(fill = FALSE) +  # to remove the legend
  #theme_bw() +             # for clean look overall
  #scale_color_continuous(guide="colorbar") +
  #scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title="Pedestrian Collisions Downtown 2010-2017") +
  theme(legend.position = "right", axis.title = element_blank(), text = element_text(size = 12)) 
...
gmap = get_map(location=CenterMidtown, source="google", zoom=15)
Serious <- c("SUSPECTED SERIOUS INJURY", "FATAL")
...
badspots_bg <-
  ...
  filter(total_bg>1) %>%
  filter(between(Latitude,CenterMidtown[2]-0.01, CenterMidtown[2]+0.01) && between(Longitude,CenterMidtown[1]-0.01, CenterMidtown[1]+0.01))
...
badmap <- badspots %>% filter(between(Latitude,CenterMidtown[2]-0.01, CenterMidtown[2]+0.01) && between(Longitude,CenterMidtown[1]-0.01, CenterMidtown[1]+0.01)) 
ggmap(gmap, extent='normal', maprange=FALSE, show.legend=FALSE) %+% badmap + aes(x = Longitude, y = Latitude) +
...
facet_wrap(TimeOfDay ~ Injury) +
  ...
  scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title="Pedestrian Collisions Midtown 2010-2017", size="Total") +
  theme(legend.position = "right", axis.title = element_blank(), text = element_text(size = 12)) 
...
gmap = get_map(location=CenterSharpstown, source="google", zoom=15)
Serious <- c("SUSPECTED SERIOUS INJURY", "FATAL")
...
badspots_bg <-
  ...
  summarise(total_bg=n()) %>%
  filter(total_bg>1) %>%
  filter(between(Latitude,CenterSharpstown[2]-0.01, CenterSharpstown[2]+0.01) && between(Longitude,CenterSharpstown[1]-0.01, CenterSharpstown[1]+0.01))
...
badmap <- badspots %>% filter(between(Latitude,CenterSharpstown[2]-0.01, CenterSharpstown[2]+0.01) && between(Longitude,CenterSharpstown[1]-0.01, CenterSharpstown[1]+0.01)) 
ggmap(gmap, extent='normal', maprange=FALSE, show.legend=FALSE) %+% badmap + aes(x = Longitude, y = Latitude) +
  geom_point(data=badspots_bg, aes(size=total_bg, x = Longitude, y = Latitude), fill="yellow", shape=21, color="black", stroke=.5, show.legend=NA) +  
  geom_point(data=badmap, aes(size=total, x = Longitude, y = Latitude, color=total)) +
facet_wrap(TimeOfDay ~ Injury) +
  ...
  scale_color_continuous(guide="colorbar", name="Subtotal") +
  scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title="Pedestrian Collisions Sharpstown 2010-2017", size="Total") +
  theme(legend.position = "right", axis.title = element_blank(), text = element_text(size = 12)) 
...
gmap = get_map(location=CenterNorthline, source="google", zoom=15)
Serious <- c("SUSPECTED SERIOUS INJURY", "FATAL")
...
badspots_bg <-
  ...
  summarise(total_bg=n()) %>%
  filter(total_bg>1) %>%
  filter(between(Latitude,CenterNorthline[2]-0.01, CenterNorthline[2]+0.01) && between(Longitude,CenterNorthline[1]-0.01, CenterNorthline[1]+0.01))
...
badmap <- badspots %>% filter(between(Latitude,CenterNorthline[2]-0.01, CenterNorthline[2]+0.01) && between(Longitude,CenterNorthline[1]-0.01, CenterNorthline[1]+0.01)) 
ggmap(gmap, extent='normal', maprange=FALSE, show.legend=FALSE) %+% badmap + aes(x = Longitude, y = Latitude) +
  geom_point(data=badspots_bg, aes(size=total_bg, x = Longitude, y = Latitude), fill="yellow", shape=21, color="black", stroke=.5, show.legend=NA) +  
  geom_point(data=badmap, aes(size=total, x = Longitude, y = Latitude, color=total)) +
facet_wrap(TimeOfDay ~ Injury) +
  ...
  scale_color_continuous(guide="colorbar", name="Subtotal") +
  scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title="Pedestrian Collisions Northline 2010-2017", size="Total") +
  theme(legend.position = "right", axis.title = element_blank(), text = element_text(size = 12)) 
