...
ChargesTemp <- DF_charges %>% group_by(Charge) %>% tally() %>% arrange(-n)
...
# Unsafe Lane Change
mask <- update_charge("CHANGE LANE")
...
DF_Charges_temp <- 
  DF_charges %>% 
  tidyr::unite("Simple_Charge", a:n, na.rm = TRUE, remove = FALSE)
...
saveRDS(df, "~/Dropbox/Rprojects/Collisions/Collisions_2010_2017.RDS")
